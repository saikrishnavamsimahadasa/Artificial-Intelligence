...
def axiom_generator_percept_sentence(t, tvec):
    ...
    for val in range(0,len(tvec)):
        # proposition_bases_perceptual_fluents = ['Stench', 'Breeze', 'Glitter', 'Bump', 'Scream']
        if val == 0:
            if tvec[val] == False: axiom_str+='~'
            axiom_str += percept_stench_str(t)
        else:
            axiom_str += ' & '
            if tvec[val] == False: axiom_str+='~'
            if val == 1: axiom_str += percept_breeze_str(t)
            if val == 2: axiom_str += percept_glitter_str(t)
            if val == 3: axiom_str += percept_bump_str(t)
            if val == 4: axiom_str += percept_scream_str(t)
...
def axiom_generator_pits_and_breezes(x, y, xmin, xmax, ymin, ymax):
    ...
    neighbour_loc = [((x+1),y), ((x-1),y), (x,(y+1)), (x,(y-1))]
    
    lhs = breeze_str(x,y)
    rhs = pit_str(x,y)
    
    for (i,j) in neighbour_loc:
        if (i <= xmax) and (i >= xmin) and (j <= ymax) and (j >= ymin):
            rhs += ' | ' + pit_str(i,j)
            
    axiom_str = '{0} <=> ({1})'.format(lhs,rhs)
    return axiom_str
...
def axiom_generator_at_most_one_wumpus(xmin, xmax, ymin, ymax):
    ...
    for loc in locations:
        ...
        for restrict_loc in locations:
            if loc != restrict_loc:
                restricted_locations.append('~' + wumpus_str(restrict_loc[0],restrict_loc[1]))
        all_axioms.append('({0} >> ({1}))'.format(wumpus_str(loc[0],loc[1]),' & '.join(restricted_locations)))

    axiom_str = ' & '.join(all_axioms)
...
def axiom_generator_only_in_one_location(xi, yi, xmin, xmax, ymin, ymax, t = 0):
    ...
    for i in range(xmin, xmax+1):
        for j in range (ymin, ymax+1):
            if (i != xi) or (j != yi):
                restricted_loc.append(state_loc_str(i, j, t))
                
    axiom_str = '{0} & ~({1})'.format(state_loc_str(xi, yi, t), "|".join(restricted_loc))
...
def axiom_generator_only_one_heading(heading = 'north', t = 0):
    ...
    if heading=='north':
        ...
        axiom_str = '{0} & ~{1} & ~{2} & ~{3}'.format(state_heading_east_str(t), state_heading_north_str(t), state_heading_south_str(t), state_heading_west_str(t))
    elif heading=='west':
        axiom_str = '{0} & ~{1} & ~{2} & ~{3}'.format(state_heading_west_str(t), state_heading_north_str(t), state_heading_east_str(t), state_heading_south_str(t))
...
def axiom_generator_at_location_ssa(t, x, y, xmin, xmax, ymin, ymax):
    ...
    possible_actions = ['({0} & ({1} | {2} | {3} | {4} | {5} | ~{6}))'.format(state_loc_str(x, y, t), action_grab_str(t), percept_bump_str(t + 1), action_shoot_str(t), action_turn_right_str(t), action_turn_left_str(t), action_forward_str(t))]
    
    #previous locations and the actions
    locs = [(((x + 1), y), 'W'), (((x - 1), y), 'E'), ((x, (y + 1)), 'S'), ((x, (y - 1)), 'N')]
    
    for ((i , j), act) in locs:
        if ((i <= xmax) and (i >= xmin) and (j <= ymax) and (j >= ymin)):
            #action-east
            if act == 'E':
                possible_actions.append('({0} & ({1} & {2}))'.format(state_loc_str(x - 1, y, t), 
            ...
            #action-south
                ...
                possible_actions.append('({0} & ({1} & {2}))'.format(state_loc_str(x, y + 1, t), 
    ...
    # successor location
    loc_successor = state_loc_str(x, y, t + 1)
    # all possible actions
    all_actions = ' | '.join(possible_actions)
    # final axiom
    axiom_str = '{0} <=> ({1})'.format(loc_successor, all_actions)
    return axiom_str
...
def axiom_generator_heading_north_ssa(t):
    ...
    axiom_str = ''
    "*** YOUR CODE HERE ***"
    #stay still heading to north
    wait_move = '({0} & (~{1} & ~{2}))'.format(state_heading_north_str(t), action_turn_right_str(t), action_turn_left_str(t))
    #turn to north from west
    right_move = '({0} & {1})'.format(state_heading_west_str(t), action_turn_right_str(t))
    #turn to north from east
    left_move = '({0} & {1})'.format(state_heading_east_str(t), action_turn_left_str(t))
    
    
    axiom_str = '{0} <=> ({1} | {2} | {3})'.format(state_heading_north_str(t+1), wait_move, left_move, right_move)
...
def axiom_generator_heading_east_ssa(t):
    ...
    #stay still heading to east
    wait_move = '({0} & (~{1} & ~{2}))'.format(state_heading_east_str(t), action_turn_right_str(t), action_turn_left_str(t))
    #turn to east from north
    right_move = '({0} & {1})'.format(state_heading_north_str(t), action_turn_right_str(t))
    #turn to east from south
    left_move = '({0} & {1})'.format(state_heading_south_str(t), action_turn_left_str(t))

    
    axiom_str = '{0} <=> ({1} | {2} | {3})'.format(state_heading_east_str(t+1), wait_move, left_move, right_move)
...
def axiom_generator_heading_south_ssa(t):
    ...
    #stay still heading to south
    wait_move = '({0} & (~{1} & ~{2}))'.format(state_heading_south_str(t), action_turn_right_str(t), action_turn_left_str(t))
    #turn to south from east
    right_move = '({0} & {1})'.format(state_heading_east_str(t), action_turn_right_str(t))
    #turn to south from west
    left_move = '({0} & {1})'.format(state_heading_west_str(t), action_turn_left_str(t))

    
    axiom_str = '{0} <=> ({1} | {2} | {3})'.format(state_heading_south_str(t+1), wait_move, left_move, right_move)
...
def axiom_generator_heading_west_ssa(t):
    ...
    #stay still heading to west
    wait_move = '({0} & (~{1} & ~{2}))'.format(state_heading_west_str(t), action_turn_right_str(t), action_turn_left_str(t))
    #turn to west from south
    right_move = '({0} & {1})'.format(state_heading_south_str(t), action_turn_right_str(t))
    #turn to west from north
    left_move = '({0} & {1})'.format(state_heading_north_str(t), action_turn_left_str(t))

    
    axiom_str = '{0} <=> ({1} | {2} | {3})'.format(state_heading_west_str(t+1), wait_move, left_move, right_move)
...
def axiom_generator_heading_only_west(t):
    ...
    axiom_str = '{0} <=> (~{1} & ~{2} & ~{3})'.format(state_heading_west_str(t), state_heading_north_str(t), state_heading_south_str(t), state_heading_east_str(t))
...
def axiom_generator_only_one_action_axioms(t):
    ...
    action_list = [action_turn_left_str(t), action_turn_right_str(t), action_wait_str(t), action_forward_str(t), action_grab_str(t), action_shoot_str(t), action_climb_str(t)]
    all_axioms=[]
    #only turn left action
    all_axioms.append('({0} <=> (~{1} & ~{2} & ~{3} & ~{4} & ~{5} & ~{6}))'.format(*action_list))
    #only turn right action
    all_axioms.append('({1} <=> (~{0} & ~{2} & ~{3} & ~{4} & ~{5} & ~{6}))'.format(*action_list))
    #no move action only
    all_axioms.append('({2} <=> (~{0} & ~{1} & ~{3} & ~{4} & ~{5} & ~{6}))'.format(*action_list))
    #forward action only
    all_axioms.append('({3} <=> (~{0} & ~{1} & ~{2} & ~{4} & ~{5} & ~{6}))'.format(*action_list))
    #grab action only
    all_axioms.append('({4} <=> (~{0} & ~{1} & ~{2} & ~{3} & ~{5} & ~{6}))'.format(*action_list))
    #shoot action only
    all_axioms.append('({5} <=> (~{0} & ~{1} & ~{2} & ~{3} & ~{4} & ~{6}))'.format(*action_list))
    #climb action only
    all_axioms.append('({6} <=> (~{0} & ~{1} & ~{2} & ~{3} & ~{4} & ~{5}))'.format(*action_list))
