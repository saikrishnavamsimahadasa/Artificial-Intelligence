...
class Expr:
    ...
    def __init__(self, op, *args):
        ...
        assert isinstance(op, str) or (isnumber(op) and (not args or len(args) == 0))
    ...
    def __call__(self, *args):
        ...
        assert is_symbol(self.op) and (not self.argslist or len(self.argslist) == 0)
        return Expr(self.op, *args)

    def __repr__(self):
        "Show something like 'P' or 'P(x, y)', or '~P' or '(P | Q | R)'"
        if len(self.argslist) == 0:         # Constant or proposition with arity 0
            return str(self.op)
        elif is_symbol(self.op):  # Functional or propositional operator
            return '%s(%s)' % (self.op, ', '.join(map(repr, self.argslist)))
        elif len(self.argslist) == 1: # Prefix operator
            return self.op + repr(self.argslist[0])
        else:                     # Infix operator
            return '(%s)' % (' '+self.op+' ').join(map(repr, self.argslist))

    def __eq__(self, other):
        """x and y are equal iff their ops and args are equal."""
        return (other is self) or (isinstance(other, Expr)
            and self.op == other.op and self.argslist == other.argslist)
    ...
    def __hash__(self):
        ...
        return hash(self.op) ^ hash(tuple(self.argslist))
...
def is_definite_clause(s):
    ...
    if is_symbol(s.op):
        ...
        antecedent, consequent = s.argslist
...
def prop_symbols(x):
    ...
    if not isinstance(x, Expr):
        ...
        return list(set(symbol for arg in x.argslist
...
def eliminate_implications(s):
    ...
    ((A & ~B) | (~A & B))
    """
    if len(s.argslist) == 0 and is_symbol(s.op): return s     ## (Atoms are unchanged.)
    args = list(map(eliminate_implications, s.argslist))
    a, b = args[0], args[-1]
    if s.op == '>>':
        return (b | ~a)
    elif s.op == '<<':
...
def move_not_inwards(s):
    ...
    if s.op == '~':
        ...
        a = s.argslist[0]
        if a.op == '~': return move_not_inwards(a.argslist[0]) # ~~A ==> A
        if a.op =='&': return associate('|', map(NOT, a.argslist))
        if a.op =='|': return associate('&', map(NOT, a.argslist))
        return s
    elif is_symbol(s.op) or len(s.argslist) == 0:
        return s
    else:
        return Expr(s.op, *map(move_not_inwards, s.argslist))
...
def distribute_and_over_or(s):
    ...
    """
    if s.op == '|':
        s = associate('|', s.argslist)
        if s.op != '|':
            return distribute_and_over_or(s)
        if len(s.argslist) == 0:
            return FALSE
        if len(s.argslist) == 1:
            return distribute_and_over_or(s.argslist[0])
        conj = find_if((lambda d: d.op == '&'), s.argslist)
        if not conj:
            return s
        others = [a for a in s.argslist if a is not conj]
        rest = associate('|', others)
        return associate('&', [distribute_and_over_or(c|rest)
                               for c in conj.argslist])
    elif s.op == '&':
        return associate('&', map(distribute_and_over_or, s.argslist))
    else:
        return s
...
def dissociate(op, args):
    ...
    def collect(subargs):
        for arg in subargs:
            if arg.op == op: collect(arg.argslist)
...
def is_variable(x):
    ...
    return isinstance(x, Expr) and not x.argslist and is_var_symbol(x.op)
