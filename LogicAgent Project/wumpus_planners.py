...
class PlanRouteProblem(search.Problem):
    ...
    def h(self,node):
        ...
        for gg in self.goals:
            hh+=manhattan_distance_with_heading(self.initial, gg)
        return hh
    ...
    def actions(self, state):
        ...
        actions = ['Grab', 'Shoot', 'Climb','TurnLeft', 'TurnRight', 'Wait']
        if head == 0 and ((xx,yy+1) in self.allowed):
            actions += ['Forward']
        elif head == 1 and ((xx-1,yy) in self.allowed):
            actions += ['Forward']
        elif head == 2 and ((xx,yy-1) in self.allowed):
            actions += ['Forward']
        elif head == 3 and ((xx+1,yy) in self.allowed):
            actions += ['Forward']
    ...
    def result(self, state, action):
        ...
        loc = (xx,yy)
        newState = tuple()
        if head == 0 and action=='Forward':
            newState = (xx,yy+1,head)
        elif head == 1 and action=='Forward':
            newState = (xx-1,yy,head)
        elif head == 2 and action=='Forward':
            newState = (xx,yy-1,head)
        elif head == 3 and action=='Forward':
            newState = (xx+1,yy,head)
        elif action == 'TurnLeft':
            newState = (xx,yy, (head+1)%4)
        elif action == 'TurnRight':
            newState = (xx,yy, (head-1)%4)
        elif action != ('Forward' or 'TurnRight' or 'TurnLeft'):
            newState = state
        #pass
        #print("On state "+str(state)+" and action "+str(action)+\
        #    " the new state will be "+str(newState))
        return newState
...
class PlanShotProblem(search.Problem):
    ...
    def goal_test(self, state):
        ...
        loc = (xx,yy)
        for gg in self.goals:
        #for gg in [self.goals[1]]:
            ggxx, ggyy = gg
            if head == 0 and yy<ggyy and xx == ggxx:
                return True
            elif head == 1 and yy==ggyy and xx > ggxx:
                return True
            elif head == 2 and yy>ggyy and xx == ggxx:
                return True
            elif head == 3 and yy==ggyy and xx < ggxx:
                return True
